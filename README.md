...
> 滑块一直过不去（日志停在「[滑块] 多次尝试后仍未通过」或长时间无交互）时，可在「新增账号 → 登录方式」改为**仅扫码登录**：网易云登录页的默认视图就是扫码登录，不会触发易盾滑块，二维码会直接显示在页面上。默认的**自动**模式会先试密码登录，失败后自动转扫码。
...
- ✅ **本地账号互助**：本机已登录账号之间轮换播放，不上传账号、歌曲和统计到任何第三方服务
- ✅ **听歌配额控制**：支持每日和每月听歌上限，听歌详情显示今日/当月听歌及被听进度
- ✅ **抢占式单浏览器**：同一时刻只跑一个浏览器，启动新任务自动强制关闭旧的，避免 profile 冲突；支持手动强制停止
- ✅ **多账号隔离**：每账号独立持久化 profile 目录，登录态复用
- ✅ **通知提醒**：企业微信 / 自定义 Webhook / PushPlus，用于扫码提醒与任务结果，可在网页中选择推送方式
- ✅ **配置即时生效**：所有全局设置在 Web 保存后立刻生效，无需重启
- ✅ **调试截图**：登录异常/风控/滑块失败时自动截图存档到 `数据目录/debug/{手机号}/`

## 互助听歌

本地互助：在账号「编辑」中开启「加入本机账号互助」，并为音乐人账号填写希望被播放的歌曲/专辑 ID。执行账号会在其他本地参与账号之间轮换目标，播放和统计只保存在本机 SQLite，不会上传到任何第三方服务。

1. 在「全局设置 → 本地账号互助」中配置每日/每月上限、播放比例和开始时间。
2. 在账号「编辑」中勾选「加入本机账号互助」，音乐人账号填写希望被播放的歌曲/专辑 ID。
3. 到达开始时间后自动执行；也可在「执行」中手动勾选「本地互助听歌」。
4. 账号列表的「听歌详情」可查看今日/本月帮助播放与被播放进度。

自动播放可能触发网易云账号风控。请合理设置频率，并自行承担使用风险。
...
| `LOGIN_METHOD` | `auto` | 旧账号未单独配置登录方式时的回退值；新账号可在新增账号弹窗中单独选择 |
...
### v2.0.7
- 修复登录接口未知响应被误判为密码失败的问题，仅拦截明确的账号/密码错误
- 每日任务登录态失效时自动重新登录，成功后重试任务；失败时发送通知（上游 [#31](https://github.com/XingHehy/netease-musician-task/issues/31)）
- 移除云端互助听歌，仅保留本地账号互助；听歌互助进度直接显示在账号卡片上
- 新增「同步」按钮：读取音乐人后台任务列表，卡片显示发布任务/播放任务真实进度
- 账号列表重构为卡片式：头像、状态徽章、运行时间/本月发布/上次登录、操作按钮组
- 新增受管理端登录保护的调试截图查看接口 `/api/debug/screenshots/...`
- 全局设置保存前校验时间格式、数值上限、登录方式与 URL 合法性
- Docker 构建升级 build-push-action v6，关闭 provenance/sbom，修复阿里云 ACR 推送
